...
find $path -regextype posix-egrep -iregex '.*\.(rw2|NEF)' | parallel -j2 "$0 -f {} -w $overwrite_jpg -r $remove_raw"
    ...
    if [ ! -e "$jpg_pathname" ] || ([ -e "$jpg_pathname" ] && [ $overwrite_jpg = true ]); then
        ...
        exiftool -overwrite_original -tagsfromfile $rawfile -all:all $jpg_pathname
        changed=true
        if [ -s "$jpg_pathname" ] && [ $remove_raw = true ]; then$
